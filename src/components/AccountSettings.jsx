...
function AccountSettings() {
  ...
  useEffect(() => {
    setStripePromise(
      loadStripe(
        "pk_test_51PfKZmKixfMhfPrWZ2P1UQnTDA7ohWcfHkcDWiIc6tniqXtBo22m28m8TQAGZqHuReZ0Uo8dWUt8CSrnzI4IEONR00iLw1yFqJ"
      )
    );
  }, []);
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        const response = await fetch(
          `http://localhost:8000/api/account/${userData.user.id}`,
          {
            method: "GET",
            headers: {
        ...
        console.log("Données utilisateur reçues (brutes) :", data);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setApiErrors(
          errorData.errors || { general: "Erreur lors de la mise à jour." }
        );
      } else {
        setSuccessMessage("Informations mises à jour avec succès !");
        const updatedData = await response.json();
        reset(updatedData.user);
        setEditing(null);
      }
    } catch (error) {
      console.error(error);
  ...
  const handleDeleteField = async (field) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/api/account/${userData.user.id}/${field}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${userData.token}`,
        ...
        <div className="col-md-8 offset-md-2">
          <div className="card account-settings">
            <div className="card-body">
              ...
              {successMessage && (
                <p className="success-message mt-3">{successMessage}</p>
              )}
            </div>
          </div>
        </div>

    ...
    try {
      if (userId) {
        const response = await fetch(
          `http://localhost:8000/api/panier/${userId}`
        );
  ...
  const handleModifierProduit = async (productId, nouvelleQuantite) => {
    ...
    if (userData && userData.user && userData.user.id) {
      ...
      try {
        ...
        const response = await fetch(
          `http://localhost:8000/api/panier/${productId}/${userId}`,
          {
            method: "PUT",
        ...
        if (response.ok) {
          ...
          setPanier((prevPanier) =>
            prevPanier.map((p) =>
              p.productId === productId
                ? { ...p, quantite: updatedPanierItem.quantite }
                : p
            )
          );
          ...
          try {
            ...
            console.error(
              "Erreur lors de l'analyse de la réponse d'erreur (PUT):",
              parseError
            );
          }
        }
  ...
  const handleSupprimerProduit = async (productId) => {
    ...
    if (userData && userData.user && userData.user.id) {
      ...
      try {
        ...
        if (response.ok) {
          setPanier((prevPanier) =>
            prevPanier.filter((p) => p.productId !== productId)
          );
        ...
        {panier.map((produit) => (
          <li key={produit.productId} className="produit-panier">
            ...
            <div className="details-produit">
              ...
              <p>{produit.prix} €</p>
              <div className="qte-bin">
                <div className="quantite">
                  <button
                    onClick={() => {
                      console.log(
                        "Type de produit.quantite :",
                        typeof produit.quantite
                      );
                ...
                <button
                  onClick={() => handleSupprimerProduit(produit.productId)}
